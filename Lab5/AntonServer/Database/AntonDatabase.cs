...
namespace AntonServer.Database 
{
    ...
    public interface IPhotosDb 
    {
        ...
        Task<postOutput> PostImage(byte[] img, CancellationToken ct, string name = "default_name");
    ...
    public class FunctionsWithDB : IPhotosDb
    {
        ...
        public async Task<postOutput> PostImage(byte[] img, CancellationToken ct, string name = "default_name")
        {
            var defaultAnswer = new postOutput {id = -1, isFound = false};
            try 
            {
                await smphore.WaitAsync();
                ...
                using (var db = new DatabaseContext()) // check if image exists in DB
                {
                    ...
                    if (res != -1)
                    {
                        return new postOutput{id = res, isFound = true};
                    }
                    ...
                    return new postOutput {id = res, isFound = false};
                }
            }
            catch (Exception ex)
            {
